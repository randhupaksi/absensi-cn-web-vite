...
export default function SupportPage() {
  ...
  const [credentials, setCredentials] =
    useState<PublicTicketCredentials | null>(() => readSavedCredentials());
  ...
  const [portal, setPortal] =
    useState<PublicSupportTicketForm["portal"]>(requestedPortal);
  ...
  const ticketQuery = useQuery({
    queryKey: supportQueryKeys.publicTicket(credentials?.referenceCode),
    queryFn: () =>
      accessPublicSupportTicket(
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      ...
      setIdentityError("");
      toast.error("Tiket belum berhasil dibuat", {
  ...
  const accessMutation = useMutation({
    ...
    onSuccess: (ticket) => {
      const next = {
        ...
        accessCode: accessInput.trim(),
      };
      saveCredentials(next);
      setCredentials(next);
      queryClient.setQueryData(
        supportQueryKeys.publicTicket(next.referenceCode),
        ticket,
      );
    },
    onError: (error) =>
      toast.error("Tiket tidak ditemukan", { description: error.message }),
  });
  ...
  const replyMutation = useMutation({
    ...
    onSuccess: (ticket) => {
      queryClient.setQueryData(
        supportQueryKeys.publicTicket(credentials?.referenceCode),
  ...
  const resetMutation = useMutation({
    ...
    onError: (error) =>
      toast.error("Sesi reset belum dapat dibuat", {
        description: error.message,
      }),
  });
  ...
  const clearCredentials = () => {
    ...
    queryClient.removeQueries({ queryKey: supportQueryKeys.publicTickets() });
    setMode("track");
  };
  ...
  return (
    <main className="relative min-h-[100svh] overflow-hidden bg-[linear-gradient(180deg,#f3fbf7_0%,#e3f3ec_52%,#edf7f3_100%)] px-3 py-4 text-slate-800 dark:bg-none dark:bg-slate-950 dark:text-slate-100 sm:px-6 sm:py-5">
      ...
      <div className="relative mx-auto w-full max-w-6xl">
        <header
          className={`mb-5 flex items-center gap-4 ${ticketView ? "pt-0" : "justify-between pr-20"}`}
        >
          <BackButton
            href={ticketView ? "/support" : loginHref}
            label="Kembali"
            fixed={false}
            onClick={ticketView ? returnToSupportHome : undefined}
          />
          {!ticketView ? (
        ...
        <section className="grid gap-6">
            ...
            <>
              <aside className="rounded-[30px] border border-emerald-200/40 bg-emerald-950 p-5 text-white shadow-[0_26px_70px_rgba(6,78,59,0.22)] dark:border-emerald-800/55 sm:p-6 lg:flex lg:items-center lg:justify-between lg:gap-10">
                <div className="lg:max-w-[52%]">
                  <span className="flex size-10 items-center justify-center rounded-2xl bg-emerald-400/15 text-emerald-300">
                  ...
                  <p className="mt-2 text-sm leading-6 text-emerald-50/75">
                    Kirim kendala, simpan kode akses, lalu pantau konfirmasi
                    admin langsung dari halaman ini.
                  </p>
                </div>
                <div className="hidden mt-4 space-y-2 sm:block lg:mt-0 lg:w-[44%]">
                  {[
                    ...
                    <div
                      key={label as string}
                      className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-emerald-50"
                    >
              ...
              <div className="space-y-5">
                ...
                <form
                  ...
                  aria-hidden={mode !== "create" || Boolean(justCreated)}
                  className={`${mode === "create" && !justCreated ? "" : "hidden"} rounded-[30px] border border-slate-200/80 bg-white/95 p-5 shadow-[0_22px_60px_rgba(15,23,42,0.09)] dark:border-slate-700 dark:bg-slate-900/90 sm:p-7`}
                >
                  <div>
                    ...
                    <h2 className="mt-2 font-heading text-2xl font-semibold">
                      Ceritakan kendala loginmu
                    </h2>
                    <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">
                      Respons tidak memastikan apakah akun terdaftar. Admin
                      tetap melakukan pemeriksaan sebelum menyetujui reset.
                    </p>
                  </div>
                  ...
                  {identityError ? (
                    <p
                      role="alert"
                      className="mt-3 rounded-lg border border-rose-300/80 bg-rose-50 px-3 py-2.5 text-sm leading-5 text-rose-800 dark:border-rose-800 dark:bg-rose-950/35 dark:text-rose-200"
                    >
                      {identityError}
                    </p>
                  ) : null}
                ...
                <div
                  aria-hidden={mode !== "track" || Boolean(ticketQuery.data)}
                  className={`${mode === "track" && !ticketQuery.data ? "" : "hidden"} rounded-[30px] border border-slate-200/80 bg-white/95 p-5 shadow-[0_22px_60px_rgba(15,23,42,0.09)] dark:border-slate-700 dark:bg-slate-900/90 sm:p-7`}
                >
                      ...
                      <div className="relative">
                        ...
                        <div className="absolute right-3 top-1/2 -translate-y-1/2">
                          <button
                            ...
                            className="flex size-8 items-center justify-center rounded-full text-slate-400 hover:bg-emerald-50 hover:text-emerald-700 dark:hover:bg-emerald-950"
                          >
                ...
                {ticketQuery.isLoading ? (
                  <div className="flex min-h-64 items-center justify-center rounded-[30px] border border-slate-200 bg-white/90 dark:border-slate-700 dark:bg-slate-900">
                    <LoaderCircle className="size-7 animate-spin text-emerald-500" />
                  </div>
                ) : null}
                {ticketQuery.error ? (
                  <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800 dark:border-rose-800 dark:bg-rose-950/40 dark:text-rose-200">
                  ...
                  <>
                    <div className="flex justify-end">
                      <button
                        ...
                        className="rounded-full border border-slate-200 bg-white/80 px-4 py-2 text-xs font-semibold text-slate-600 transition hover:border-emerald-300 hover:text-emerald-700 active:scale-[0.98] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-emerald-700 dark:hover:text-emerald-300"
                      >
                        Lacak tiket lain
                      </button>
                    </div>
                    <TicketConversation
                      ...
                      onStartReset={() => resetMutation.mutate()}
                      isStartingReset={resetMutation.isPending}
          ...
          {ticketView ? (
            <div className="space-y-5">
              {justCreated && !ticketQuery.data ? (
                <CredentialCard
                  accessCode={justCreated.access_code}
                  onContinue={() => createMutation.reset()}
                />
              ) : null}
              {ticketQuery.isLoading ? (
                <div className="flex min-h-64 items-center justify-center rounded-[30px] border border-slate-200 bg-white/90 dark:border-slate-700 dark:bg-slate-900">
              ...
              {ticketQuery.data && credentials ? (
                <AccessCodeCard accessCode={credentials.accessCode} />
              ) : null}
              {ticketQuery.data ? (
                <TicketConversation
                  ticket={ticketQuery.data}
    ...
    <section className="rounded-[30px] border border-emerald-300 bg-white/95 p-5 shadow-[0_22px_60px_rgba(15,23,42,0.09)] dark:border-emerald-800 dark:bg-slate-900/95 sm:p-7">
      <div className="flex gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-400">
            Simpan satu kode ini. Kode akses adalah kunci untuk membuka kembali
            tiketmu.
          </p>
        </div>
      </div>
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950/60">
        ...
        <div className="mt-2 flex items-center justify-between gap-4">
          <code className="min-w-0 break-all text-lg font-semibold tracking-[0.08em] text-slate-900 dark:text-white">
            {accessCode}
          </code>
          <Button
            type="button"
            variant="success"
            className="h-10 shrink-0 rounded-xl px-4 shadow-none hover:shadow-none focus-visible:shadow-none"
            onClick={copy}
          >
    ...
    <section className="rounded-[24px] border border-slate-200/80 bg-white/95 p-4 shadow-[0_14px_36px_rgba(15,23,42,0.07)] dark:border-slate-700 dark:bg-slate-900/90 dark:shadow-none sm:p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <code className="mt-1 block break-all text-lg font-semibold tracking-[0.08em] text-slate-900 dark:text-white">
            {accessCode}
          </code>
        </div>
        <Button
