...
export function MapelSessionPage() {
  ...
  const sessionId =
    explicitSessionId ?? autoSessionQuery.data?.session_id ?? null;
  ...
  const invalidate = () => {
    ...
    queryClient.invalidateQueries({
      queryKey: ["subject-attendance-overview"],
    });
  ...
  const draftMutation = useMutation({
    ...
    onSuccess: () => {
      setPendingOverrides({});
      toast.success(
        "Draft sesi berhasil disimpan. Sesi belum masuk rekap resmi.",
      );
      invalidate();
    },
  ...
  const stats = useMemo(() => {
    const statuses = records.map(
      (r) => pendingOverrides[r.student_id] ?? r.status_mapel,
    );
    return {
        ...
        <>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-slate-500">
                    {session.hari.charAt(0).toUpperCase() +
                      session.hari.slice(1)}{" "}
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                  ...
                  {!isValidated && (
                    <button
                      ...
                      disabled={
                        validateMutation.isPending || draftMutation.isPending
                      }
              ...
              <section className="rounded-[24px] border border-white/70 bg-white/88 p-4 shadow-[0_24px_52px_rgba(150,163,184,0.12)] dark:border-slate-700 dark:bg-slate-900 dark:shadow-none sm:rounded-[32px] sm:p-5">
                ...
                {overviewQuery.isLoading ? (
                  ...
                  <EmptyState
                    ...
                    description="Pastikan siswa terdaftar di kelas ini."
                  />
                ) : (
                  <>
                                ...
                                <DataTableCell className="max-w-[260px] whitespace-normal text-sm">
                                  {r.is_edited ? (
                                    <span className="line-clamp-2">
                                      {r.alasan_edit ||
                                        "Sudah dikoreksi oleh guru mapel."}
                                    </span>
                                  ) : r.keterangan ? (
                                    <span className="line-clamp-2">
                                      {r.keterangan}
                        ...
                        return (
                          <MobileDataCard key={r.student_id}>
                            <MobileDataHeader
                              ...
                              badge={
                                <span
                                  className={`rounded-full px-2.5 py-1 text-xs font-semibold ${STATUS_MAPEL_CLS[effective] ?? "bg-slate-100 text-slate-600"}`}
                                >
                                  {STATUS_LABELS[effective] ?? effective}
                                </span>
                              }
                            ...
                            <div className="mt-4 grid gap-3">
                              ...
                              <MobileDataField
                                ...
                                value={
                                  <span
                                    className={`rounded-full px-2.5 py-1 text-xs font-semibold ${STATUS_PAGI_CLS[r.status_pagi] ?? "bg-slate-100 text-slate-600"}`}
                                  >
                                    {STATUS_LABELS[r.status_pagi] ??
                                      r.status_pagi}
                                  </span>
                              ...
                              <MobileDataField
                                ...
                                value={
                                  r.is_edited
                                    ? r.alasan_edit ||
                                      "Sudah dikoreksi oleh guru mapel."
                                    : r.keterangan || "Tidak ada catatan"
                                }
                              />
                            </div>
                            <MobileDataFooter>
          ...
          {koreksiTarget && (
            <KoreksiModal
              ...
              currentStatus={
                STATUS_LABELS[koreksiTarget.status_mapel] ??
                koreksiTarget.status_mapel
              }
