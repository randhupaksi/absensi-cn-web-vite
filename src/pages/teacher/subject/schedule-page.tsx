...
const SubjectScheduleReportModal = dynamic(
  () =>
    import("@/features/reports/subject/schedule-report-modal").then(
      (module) => module.SubjectScheduleReportModal,
    ),
  { ssr: false },
);
...
export function MapelSchedulePage() {
  ...
  const classFilterOptions = useMemo(() => {
    ...
    activeAssignments.forEach((assignment) => {
      assignment.schedules
        ...
        .forEach((schedule) =>
          classes.set(schedule.class_id, schedule.class_name),
        );
    });
  ...
  const dayFilterOptions = Object.entries(HARI_LABEL).map(([value, label]) => ({
    value,
    label,
  }));
  const activeSubjectCount = new Set(
    activeAssignments.map((assignment) => assignment.subject_id),
  ).size;
              ...
              <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div className="max-w-2xl">
                  ...
                  <p className="mt-3 text-sm leading-6 text-slate-600 sm:text-base">
                    ...
                    ke ticket absensi ketika kelas sudah dimulai.
                  </p>
                </div>
              </div>
            </section>
            ...
            {allTickets.length === 0 ? (
              <section className="rounded-[32px] border border-white/70 bg-white/88 p-6 shadow-[0_24px_52px_rgba(150,163,184,0.12)]">
                <EmptyState
                  ...
                  description="Jadwal mengajar akan muncul setelah admin menautkan mata pelajaran dan slot waktu yang aktif."
                />
              </section>
            ) : (
              <section className="rounded-[32px] border border-white/70 bg-white/88 p-5 shadow-[0_24px_52px_rgba(150,163,184,0.12)] sm:p-6">
                  ...
                  <div className="mb-6 flex items-center gap-3 rounded-[20px] border border-amber-200/80 bg-amber-50/70 px-4 py-3.5 text-sm text-amber-900">
                    ...
                    <p className="leading-6">
                      <span className="font-semibold">
                        Hari ini libur{holidayName ? `: ${holidayName}` : ""}.
                      </span>{" "}
                ...
                <div className="mb-6 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                  {groupSchedulesBySubject(activeAssignments).map((group) => (
                    <div
                      key={group.subjectId}
                      className="min-w-0 rounded-[18px] border border-emerald-100/80 bg-emerald-50/45 px-3.5 py-3"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <p className="truncate text-sm font-semibold text-slate-900">
                          {group.subjectName}
                        </p>
                        <span className="shrink-0 rounded-full bg-white px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-emerald-700">
                          {group.classes.length} kelas
                        </span>
                      </div>
                      <p className="mt-1 truncate text-xs text-slate-500">
                        {group.classes.map((item) => item.name).join(" · ")}
                      </p>
                    </div>
                  ))}
                </div>
                ...
                {tickets.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {groupTicketsByDate(tickets).map(
                      ([dateKey, dayTickets]) => (
                        <div
                          key={dateKey}
                        ...
                        >
                          <div className="mb-3 flex items-center gap-3">
                            <span
                              className={`flex size-10 shrink-0 flex-col items-center justify-center rounded-[14px] ${getDayGroupTone(dayTickets[0].date).dateBadge}`}
                            >
                              <span className="text-[10px] font-bold uppercase leading-none">
                                {format(dayTickets[0].date, "MMM", {
                                  locale: localeID,
                                })}
                              </span>
                              <span className="mt-0.5 text-sm font-bold leading-none">
                                {format(dayTickets[0].date, "d")}
                              </span>
                            </span>
                            <div>
                              <p
                                className={`text-sm font-semibold ${getDayGroupTone(dayTickets[0].date).heading}`}
                              >
                                {format(dayTickets[0].date, "EEEE, d MMMM", {
                                  locale: localeID,
                                })}
                              </p>
                              <p className="text-xs text-slate-500">
                                {dayTickets.length} ticket jadwal
                              </p>
                            </div>
                            {dateKey === today ? (
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-2 flex min-w-0 items-baseline gap-2">
            <p className="shrink-0 truncate text-2xl font-bold tracking-[-0.03em] text-slate-950">
              {value}
            </p>
            {detail ? (
        ...
        <span
          className={`flex size-10 shrink-0 items-center justify-center rounded-[14px] ${tones[tone]}`}
        >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span
              className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${state.cls}`}
            >
              {state.label}
            </span>
          </div>
  ...
  return Array.from({ length: 7 }, (_, dayOffset) => {
    ...
    return assignments.flatMap((assignment) => {
      ...
      const schedules = assignment.schedules
        .filter(
          (schedule) =>
            schedule.is_active && schedule.hari.toLowerCase() === day,
        )
        .sort((first, second) =>
          normalizeTime(first.jam_mulai).localeCompare(
            normalizeTime(second.jam_mulai),
          ),
        );
      ...
      schedules.forEach((schedule) => {
        ...
        if (
          current &&
          current.end === start &&
          current.classId === schedule.class_id
        ) {
          current.end = end;
