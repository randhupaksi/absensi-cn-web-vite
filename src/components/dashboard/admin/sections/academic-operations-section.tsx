  ...
  return <>
    <section className="relative mt-6 overflow-hidden rounded-[30px] border border-white/75 bg-white/95 p-4 shadow-[0_28px_80px_rgba(28,77,61,0.1)] sm:p-5 lg:p-6">
      <div className="flex flex-col gap-5 border-b border-slate-200/80 pb-5 lg:flex-row lg:items-start lg:justify-between">
        ...
        <Button className="h-12 rounded-[18px] bg-emerald-700 px-5 text-white" onClick={() => setCreateOpen(true)}><Plus className="size-4" />Tambah Data</Button>
      </div>
      ...
      <Tabs value={tab} onValueChange={(value) => { setTab(value as Tab); setCreateOpen(false); }} className="mt-5">
        ...
        <TabsContent value="offerings" className="mt-4"><DataTableCard isLoading={isLoading} columnCount={7} isEmpty={offerings.length === 0} emptyTitle="Belum ada penawaran mapel" emptyDescription="Tambahkan mapel ke kelas dan tahun ajaran." icon={BookPlus}><table className="min-w-full text-left text-sm"><Head labels={["Mapel", "Kelas", "Unit", "Tahun", "JP/Minggu", "Status", "Aksi"]} /><tbody>{offerings.map((item) => <tr key={item.id} className="border-t border-slate-100"><Cell><b>{item.subject_code}</b><small>{item.subject_name}</small></Cell><Cell>{item.class_name}</Cell><Cell><Pill>{item.school_unit_code}</Pill></Cell><Cell>{item.school_year_name}</Cell><Cell>{item.weekly_hours} JP</Cell><Cell><Status active={item.is_active} /></Cell><Cell><ActionButtons onEdit={() => setEditingOffering(item)} onDelete={() => setDeleteTarget({ id: item.id, label: `${item.subject_code} · ${item.class_name}`, kind: "offerings" })} /></Cell></tr>)}</tbody></table></DataTableCard></TabsContent>
        <TabsContent value="rooms" className="mt-4"><DataTableCard isLoading={isLoading} columnCount={6} isEmpty={rooms.length === 0} emptyTitle="Belum ada ruangan" emptyDescription="Tambahkan ruang yang dapat dipakai jadwal." icon={DoorOpen}><table className="min-w-full text-left text-sm"><Head labels={["Kode", "Nama", "Unit", "Tipe", "Kapasitas", "Aksi"]} /><tbody>{rooms.map((item) => <tr key={item.id} className="border-t border-slate-100"><Cell><b>{item.code}</b></Cell><Cell>{item.name}</Cell><Cell><Pill>{item.school_unit_code}</Pill></Cell><Cell>{item.room_type}</Cell><Cell>{item.capacity}</Cell><Cell><ActionButtons onEdit={() => setEditingRoom(item)} onDelete={() => setDeleteTarget({ id: item.id, label: item.name, kind: "rooms" })} /></Cell></tr>)}</tbody></table></DataTableCard></TabsContent>
        <TabsContent value="overrides" className="mt-4"><DataTableCard isLoading={isLoading} columnCount={6} isEmpty={overrides.length === 0} emptyTitle="Belum ada perubahan jadwal" emptyDescription="Jadwal normal tetap berlaku selama tidak ada override." icon={CalendarSync}><table className="min-w-full text-left text-sm"><Head labels={["Jadwal", "Tanggal Asal", "Jenis", "Pengganti", "Status", "Aksi"]} /><tbody>{overrides.map((item) => { const schedule = schedules.find((x) => x.id === item.schedule_id); return <tr key={item.id} className="border-t border-slate-100"><Cell><b>{schedule?.subject_code ?? "Jadwal"}</b><small>{schedule?.class_name ?? item.schedule_id}</small></Cell><Cell>{item.original_date}</Cell><Cell><Pill>{item.override_type}</Pill></Cell><Cell>{item.replacement_date || item.replacement_room_id || item.substitute_teacher_id || "—"}</Cell><Cell><Status active={item.status === "ACTIVE"} /></Cell><Cell><ActionButtons onEdit={() => setEditingOverride(item)} onDelete={() => setDeleteTarget({ id: item.id, label: `${item.override_type} · ${item.original_date}`, kind: "overrides" })} /></Cell></tr>; })}</tbody></table></DataTableCard></TabsContent>
      </Tabs>
    </section>
...
function Head({ labels }: { labels: string[] }) { return <thead><tr className="bg-emerald-50/60 text-slate-700">{labels.map((label) => <th key={label} className="border-b border-emerald-100 px-4 py-4 font-medium">{label}</th>)}</tr></thead>; }
function Cell({ children }: { children: React.ReactNode }) { return <td className="px-4 py-4 text-slate-600">{children}</td>; }
