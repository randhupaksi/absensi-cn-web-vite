import { ActionButtons, AddButton, DataTable, DataTableBody, DataTableCard, DataTableCell, DataTableHeadRow, DataTableRow, SectionTabSwitch, StatCard, StatusBadge, usePagination } from "@/components/dashboard/admin/sections/section-ui";
...
import { Building2, Network, Save, School } from "lucide-react";
...
export function AcademicStructureSection({ units, programs, isLoading }: { units: AdminSchoolUnit[]; programs: AdminMajor[]; isLoading: boolean }) {
  ...
  const { pageItems: pagePrograms, pagination: programsPagination } = usePagination(programs);
  ...
  return <>
    <section className="mt-6 rounded-[30px] border border-white/75 bg-white/95 p-4 shadow-[0_28px_80px_rgba(28,77,61,0.1)] sm:p-5 lg:p-6">
      ...
      <SectionTabSwitch tabs={[{ value: "units", label: "Unit Sekolah", icon: School }, { value: "programs", label: "Program/Jurusan", icon: Network }]} />
        ...
        <TabsContent value="units" className="mt-4"><DataTableCard isLoading={isLoading} columnCount={5} isEmpty={!units.length} emptyTitle="Belum ada unit sekolah" emptyDescription="Tambahkan SMA atau SMK." icon={School} pagination={unitsPagination}><DataTable><DataTableHeadRow labels={["Kode", "Nama", "Jenjang", "Status", "Aksi"]} /><DataTableBody>{pageUnits.map((item) => <DataTableRow key={item.id}><DataTableCell><b>{item.code}</b></DataTableCell><DataTableCell>{item.name}</DataTableCell><DataTableCell>{item.education_level}</DataTableCell><DataTableCell><StatusBadge isActive={item.is_active} /></DataTableCell><DataTableCell><ActionButtons onEdit={() => setUnit(item)} onDelete={() => setDeleting({ id: item.id, label: item.name, kind: "units" })} /></DataTableCell></DataTableRow>)}</DataTableBody></DataTable></DataTableCard></TabsContent>
        <TabsContent value="programs" className="mt-4"><DataTableCard isLoading={isLoading} columnCount={6} isEmpty={!programs.length} emptyTitle="Belum ada program" emptyDescription="Tambahkan jurusan SMK atau program umum SMA." icon={Building2} pagination={programsPagination}><DataTable><DataTableHeadRow labels={["Kode", "Nama", "Unit", "Tipe", "Status", "Aksi"]} /><DataTableBody>{pagePrograms.map((item) => <DataTableRow key={item.id}><DataTableCell><b>{item.code}</b></DataTableCell><DataTableCell>{item.name}</DataTableCell><DataTableCell><Badge variant="outline" className="border-emerald-200 bg-emerald-50 text-emerald-700">{item.school_unit_code}</Badge></DataTableCell><DataTableCell>{item.program_type}</DataTableCell><DataTableCell><StatusBadge isActive={item.is_active} /></DataTableCell><DataTableCell><ActionButtons onEdit={() => setProgram(item)} onDelete={() => setDeleting({ id: item.id, label: item.name, kind: "programs" })} /></DataTableCell></DataTableRow>)}</DataTableBody></DataTable></DataTableCard></TabsContent>
      </Tabs>
    </section>
      {((tab === "units" && createOpen) || unit) && <SchoolUnitModal open item={unit} pending={pending} onOpenChange={(open) => { if (!open) { setCreateOpen(false); setUnit(null); } }} onSubmit={(values) => unit ? editUnit.mutate({ id: unit.id, values }) : createUnit.mutate(values)} />}
      {((tab === "programs" && createOpen) || program) && <ProgramModal open item={program} units={units} pending={pending} onOpenChange={(open) => { if (!open) { setCreateOpen(false); setProgram(null); } }} onSubmit={(values) => program ? editProgram.mutate({ id: program.id, values }) : createProgram.mutate(values)} />}
...
function Footer({ formId, pending, onCancel }: { formId: string; pending: boolean; onCancel: () => void }) { return <div className="flex justify-end gap-3"><Button variant="outline" className="transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-400 hover:bg-slate-200 hover:text-slate-950 hover:shadow-[0_14px_28px_rgba(15,23,42,0.14)] active:translate-y-0 active:scale-[0.96] active:bg-slate-300" onClick={onCancel} disabled={pending}>Batal</Button><Button type="submit" form={formId} className="bg-emerald-700 text-white shadow-[0_20px_40px_rgba(22,101,52,0.2)] transition-all duration-200 hover:bg-emerald-800 active:scale-[0.96] active:bg-emerald-900" disabled={pending}><Save className="size-4" />{pending ? "Menyimpan..." : "Simpan"}</Button></div>; }
