...
import { BookPlus, CalendarSync, DoorOpen, Sparkles } from "lucide-react";
...
function Footer({ formId, pending, onCancel }: { formId: string; pending: boolean; onCancel: () => void }) {
  return <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
    <Button type="button" variant="outline" className="h-12 rounded-[1.1rem] px-5" onClick={onCancel} disabled={pending}>Batal</Button>
    <Button type="submit" form={formId} className="h-12 rounded-[1.1rem] bg-emerald-700 px-5 text-white" disabled={pending}><Sparkles className="size-4" />{pending ? "Menyimpan..." : "Simpan"}</Button>
  </div>;
}

export function SubjectOfferingModal({ open, item, subjects, classes, schoolYears, pending, onOpenChange, onSubmit }: {
  open: boolean; item: AdminSubjectOffering | null; subjects: AdminSubject[]; classes: AdminClass[]; schoolYears: AdminSchoolYear[]; pending: boolean;
  onOpenChange: (open: boolean) => void; onSubmit: (values: SubjectOfferingFormValues) => void;
}) {
  const form = useForm<SubjectOfferingFormValues>({ resolver: zodResolver(subjectOfferingSchema), defaultValues: offeringValues(item) });
  useEffect(() => { if (open) form.reset(offeringValues(item)); }, [form, item, open]);
  return <PremiumModal open={open} onOpenChange={onOpenChange} icon={BookPlus} title={item ? "Edit Penawaran Mapel" : "Tambah Penawaran Mapel"} description="Tentukan mapel yang tersedia untuk satu kelas pada tahun ajaran tertentu." className="sm:!max-w-2xl" footer={<Footer formId="offering-form" pending={pending} onCancel={() => onOpenChange(false)} />}>
    <form id="offering-form" onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 sm:grid-cols-2">
      <SelectField label="Mata Pelajaran" value={form.watch("subject_id")} onChange={(value) => form.setValue("subject_id", value, { shouldValidate: true })} options={subjects.filter((x) => x.is_active).map((x) => ({ value: x.id, label: x.name, description: `${x.code} · ${x.scope}` }))} error={form.formState.errors.subject_id?.message} />
      <SelectField label="Kelas" value={form.watch("class_id")} onChange={(value) => { form.setValue("class_id", value, { shouldValidate: true }); const selected = classes.find((x) => x.id === value); if (selected) form.setValue("school_year_id", selected.school_year_id); }} options={classes.filter((x) => x.is_active).map((x) => ({ value: x.id, label: x.display_name, description: `${x.school_unit_code} · ${x.school_year_name}` }))} error={form.formState.errors.class_id?.message} />
      <SelectField label="Tahun Ajaran" value={form.watch("school_year_id")} onChange={(value) => form.setValue("school_year_id", value, { shouldValidate: true })} options={schoolYears.map((x) => ({ value: x.id, label: x.name }))} error={form.formState.errors.school_year_id?.message} />
      <div className={premiumModalFieldClassName}><label className={premiumModalLabelClassName}>JP per Minggu</label><Input type="number" min={1} className={inputClass} {...form.register("weekly_hours", { valueAsNumber: true })} /><FieldError message={form.formState.errors.weekly_hours?.message} /></div>
      <BooleanField control={form.control} name="is_required" label="Sifat Mapel" trueLabel="Wajib" falseLabel="Pilihan" />
...
function BooleanField<T extends RoomFormValues | SubjectOfferingFormValues>({ control, name, label, trueLabel, falseLabel }: { control: ReturnType<typeof useForm<T>>["control"]; name: "is_active" | "is_required"; label: string; trueLabel: string; falseLabel: string }) {
...
function offeringValues(item: AdminSubjectOffering | null): SubjectOfferingFormValues { return { subject_id: item?.subject_id ?? "", class_id: item?.class_id ?? "", school_year_id: item?.school_year_id ?? "", weekly_hours: item?.weekly_hours ?? 2, is_required: item?.is_required ?? true, is_active: item?.is_active ?? true }; }
