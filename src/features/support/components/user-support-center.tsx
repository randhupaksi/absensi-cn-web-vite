...
import { ModalActions, SearchFilterBar } from "@/features/admin/management/shared/section-ui";
...
import { BellRing, CheckCircle2, ChevronLeft, ChevronRight, CircleAlert, Headphones, LifeBuoy, LoaderCircle, MessageSquareText, MessageSquarePlus, Plus, ShieldCheck } from "lucide-react";
...
export function UserSupportCenter() {
  ...
  const ticketsQuery = useQuery({
    queryKey: ["my-support-tickets", query.trim(), ticketOffset],
    queryFn: () => getMySupportTickets({ q: query.trim() || undefined, limit: ticketPageSize, offset: ticketOffset }),
  ...
  const openNotification = (notificationID: string, actionURL: string) => {
    const reference = new URL(normalizeAppRoute(actionURL), window.location.origin).searchParams.get("ticket");
    ...
    <div className="space-y-5">
      <section className="overflow-hidden rounded-[30px] border border-white/70 bg-[radial-gradient(circle_at_top_right,rgba(110,231,183,0.25),transparent_26%),linear-gradient(135deg,#ffffff_0%,#f0faf5_100%)] p-4 shadow-[0_22px_55px_rgba(15,23,42,0.08)] dark:border-slate-700 dark:bg-none dark:bg-slate-900 dark:shadow-none sm:p-7">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:gap-4">
            <span className="flex size-11 shrink-0 items-center justify-center rounded-[16px] bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300 sm:size-12 sm:rounded-[18px]"><LifeBuoy className="size-5 sm:size-6" /></span>
            <div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-600 dark:text-emerald-300">Pusat bantuan</p><h1 className="mt-1 max-w-full font-heading text-2xl font-semibold leading-tight text-slate-950 dark:text-white sm:text-3xl">Percakapan langsung dengan admin</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500 dark:text-slate-400">Sampaikan kendala, pantau status, dan simpan seluruh jawaban dalam satu tiket.</p></div>
          </div>
          <div className="flex w-full items-center justify-end gap-2 sm:w-auto">
            <Popover>
              <PopoverTrigger
                render={<Button type="button" variant="outline" aria-label="Buka notifikasi tiket" />}
              ...
              >
                <PopoverHeader className="flex-row items-start justify-between gap-3 border-b border-slate-200/80 px-3 pb-3 pt-2 dark:border-slate-700">
                  <div className="flex min-w-0 items-start gap-2.5">
                    ...
                    <div className="min-w-0">
                      <PopoverTitle className="text-sm font-semibold text-slate-900 dark:text-slate-100">Notifikasi tiket</PopoverTitle>
                      <p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">Balasan admin tersimpan di sini.</p>
                    </div>
                  </div>
                  {(notificationsQuery.data?.unread_count ?? 0) > 0 ? (
                ...
                <div className="max-h-80 space-y-2 overflow-y-auto px-1 py-2">
                  {notificationsQuery.isLoading ? (
                    <div className="flex items-center justify-center px-3 py-8"><LoaderCircle className="size-5 animate-spin text-emerald-500" /></div>
                    ...
                    notificationsQuery.data.notifications.slice(0, 6).map((notification) => (
                      ...
                      >
                        <div className="flex items-start gap-2.5">
                          <span className={`mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-lg ${notification.read ? "bg-slate-200/80 text-slate-500 dark:bg-slate-800 dark:text-slate-300" : "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/70 dark:text-emerald-200"}`}>
                            <NotificationIcon notification={notification} />
                          </span>
                          <div className="min-w-0 flex-1">
                            <div className="flex items-start justify-between gap-2">
                              <p className="line-clamp-1 min-w-0 text-xs font-semibold text-slate-900 dark:text-slate-100">{notification.title}</p>
                              {!notification.read ? <span className="mt-1 size-1.5 shrink-0 rounded-full bg-sky-500" aria-label="Belum dibaca" /> : null}
                            </div>
                            <p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-600 dark:text-slate-300">{notification.description}</p>
                            <p className="mt-1.5 text-[10px] text-slate-400">{formatSupportDate(notification.created_at)}</p>
                          </div>
                        </div>
                      </button>
                    ))
                    ...
                    <div className="px-3 py-8 text-center">
                      ...
                      <p className="mt-2 text-xs font-semibold text-slate-600 dark:text-slate-300">Belum ada notifikasi tiket</p>
                    </div>
                  )}
                </div>
              </PopoverContent>
            </Popover>
            <Button type="button" variant="success" className="h-11 min-w-0 flex-1 rounded-2xl px-3 text-sm font-semibold sm:flex-none sm:px-5" onClick={() => setShowCreate(true)}><Plus /> Buat tiket</Button>
          </div>
        </div>
      </section>
      ...
      <PremiumModal
        ...
        footer={
          <ModalActions
            ...
            onSubmit={() => void form.handleSubmit((values) => createMutation.mutate(values))()}
            submitLabel="Kirim tiket"
            submitIcon={MessageSquarePlus}
          />
        }
      >
        <form id="teacher-support-ticket-form" onSubmit={form.handleSubmit((values) => createMutation.mutate(values))} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <RadixSelectField
                    ...
                    triggerClassName={form.formState.errors.category ? "h-14 !border-rose-500 focus-visible:!border-rose-500 focus-visible:!ring-rose-200 dark:!border-rose-400 dark:focus-visible:!ring-rose-400/25" : "h-14"}
                  />
                )}
              />
            ...
            <div className={premiumModalFieldClassName}>
              <label htmlFor="teacher-support-subject" className={premiumModalLabelClassName}>Judul kendala</label>
              <Input
          ...
          <div className={premiumModalFieldClassName}>
            <label htmlFor="teacher-support-message" className={premiumModalLabelClassName}>Detail kendala</label>
            <Textarea
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="ml-auto text-xs text-slate-400">{form.watch("message").length}/1500</span>
            </div>
          </div>
        </form>
      </PremiumModal>
      ...
      <section className="grid min-h-[560px] gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="h-fit self-start rounded-[28px] border border-slate-200/80 bg-white/90 p-4 shadow-[0_18px_45px_rgba(15,23,42,0.07)] dark:border-slate-700 dark:bg-slate-900/90 dark:shadow-none">
          <SearchFilterBar value={query} onChange={(value) => { setQuery(value); setTicketOffset(0); }} placeholder="Cari kode atau judul tiket" className="w-full" />
          <div className="mt-4 max-h-[650px] space-y-2 overflow-y-auto pr-1">
            ...
            {filteredTickets.map((ticket) => <button key={ticket.reference_code} type="button" onClick={() => setSearchParams({ ticket: ticket.reference_code })} className={`w-full rounded-[20px] border p-4 text-left transition-[border-color,transform] hover:border-emerald-300 dark:hover:!border-emerald-400/70 active:scale-[0.985] ${selectedReference === ticket.reference_code ? "border-emerald-400 bg-emerald-50 dark:border-emerald-700 dark:bg-emerald-950/35" : "border-slate-200 bg-slate-50/70 dark:border-slate-700 dark:bg-slate-950/45"}`}><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-bold uppercase tracking-[0.13em] text-emerald-600 dark:text-emerald-300">{ticket.reference_code}</span>{ticket.unread ? <span className="size-2 rounded-full bg-rose-500" aria-label="Ada balasan baru" /> : null}</div><p className="mt-2 line-clamp-2 text-sm font-semibold text-slate-900 dark:text-white">{ticket.subject}</p><div className="mt-3 flex items-center justify-between gap-2"><SupportStatusBadge status={ticket.status} /><span className="text-[10px] text-slate-400">{formatSupportDate(ticket.last_message_at)}</span></div></button>)}
          </div>
...
function NotificationIcon({ notification }: { notification: SupportNotification }) {
  if (notification.priority === "warning" || notification.type.toLowerCase().includes("warning")) {
    return <CircleAlert className="size-3.5" />;
  }
  if (notification.type.toLowerCase().includes("resolved") || notification.type.toLowerCase().includes("complete")) {
    return <CheckCircle2 className="size-3.5" />;
  }
  if (notification.type.toLowerCase().includes("reply") || notification.type.toLowerCase().includes("message")) {
