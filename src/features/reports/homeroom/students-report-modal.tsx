...
type DailyStudentRecord = StaffAttendanceRecord &
  Pick<StaffStudentSummary, "gender" | "nisn" | "is_active">;
...
async function generateWalasSiswaPdf(
  ...
  doc.save(
    `Laporan-Walas-Siswa-${homeroom.class_name.replace(/\s+/g, "-")}-${new Date().toISOString().slice(0, 10)}.pdf`,
  );
}
    ...
    columns: [
      ...
      ...(columns.nis
        ? [
            {
              header: "NIS",
              value: (student: StaffStudentSummary) => student.nis,
              width: 17,
            },
            {
              header: "NISN",
...
async function generateDailyWalasSiswaPdf(
  ...
  const body = data.map((student, index) => {
    ...
    if (columns.checkin) {
      row.push(
        student.check_in_at
          ? new Date(student.check_in_at).toLocaleTimeString("id-ID", {
              hour: "2-digit",
              minute: "2-digit",
            })
          : "â€”",
...
async function generateDailyWalasSiswaExcel(
  ...
  await exportStyledExcelReport({
    ...
    columns: [
      ...
      {
        header: "Tanggal",
        value: (student) => new Date(`${student.attendance_date}T00:00:00`),
        width: 16,
        kind: "date",
      },
      ...(columns.nis
        ? [
            {
              header: "NIS",
              value: (student: DailyStudentRecord) => student.nis,
              width: 17,
            },
          ]
        : []),
      ...(columns.gender
        ? [
            {
              header: "Jenis Kelamin",
              value: (student: DailyStudentRecord) =>
                student.gender === "MALE"
                  ? "Laki-laki"
                  : student.gender === "FEMALE"
                    ? "Perempuan"
      ...
      ...(columns.nisn
        ? [
            {
              header: "NISN",
              value: (student: DailyStudentRecord) => student.nisn,
              width: 17,
            },
          ]
        : []),
        ...
        ? [
            {
              header: "Absen Masuk",
              value: (student: DailyStudentRecord) =>
                student.check_in_at ? new Date(student.check_in_at) : null,
              width: 20,
              kind: "date" as const,
              numberFormat: "hh:mm",
            },
...
function buildCumulativeStudents(
  ...
  records.forEach((record) => {
    ...
    switch (record.status.toLowerCase()) {
      case "hadir":
        ...
        break;
      case "izin":
        student.permission_count += 1;
        break;
      case "sakit":
        student.sick_count += 1;
        break;
      case "alfa":
        student.alpha_count += 1;
        break;
    }
  });
...
export function WalasSiswaReportModal({ open, onOpenChange, homeroom }: Props) {
  ...
  const [conditionFilter, setConditionFilter] =
    useState<ConditionFilter | null>(null);
  const [columns, setColumns] = useState<CumulativeColumns>({
    nis: true,
    gender: false,
    identitas: false,
    hadir: true,
  ...
  const [dailyColumns, setDailyColumns] = useState<DailyColumns>({
    nis: true,
    gender: false,
    nisn: false,
    status: true,
  ...
  const periodAnswered =
    dateMode === "today" ||
    (dateMode === "specific" && Boolean(specificDate)) ||
    (dateMode === "range" &&
      Boolean(rangeFrom) &&
      Boolean(rangeTo) &&
      rangeValid);
  ...
  async function handleDownload() {
    ...
    try {
      ...
      const conditionLabel = conditionFilter
      ...
      if (reportType === "daily") {
        const studentById = new Map(
          (periodStudents ?? []).map((student) => [student.id, student]),
        );
        let filtered: DailyStudentRecord[] = (overview.records ?? []).map(
          (record) => {
        ...
        if (conditionFilter === "perlu_perhatian")
          filtered = filtered.filter(
            (student) => student.status.toLowerCase() === "alfa",
          );
        if (conditionFilter === "stabil")
          filtered = filtered.filter(
            (student) => student.status.toLowerCase() !== "alfa",
          );
        ...
        if (filtered.length === 0) {
          toast.warning(
            "Tidak ada data absensi yang sesuai periode dan filter.",
          );
          return;
        }
        ...
        if (format === "excel")
          await generateDailyWalasSiswaExcel(
            sorted,
            homeroom,
            periodLabel,
            conditionLabel,
          ...
          await generateDailyWalasSiswaPdf(
            sorted,
            homeroom,
            periodLabel,
            conditionLabel,
      ...
      let cumulative = buildCumulativeStudents(
        periodStudents ?? [],
        overview.records ?? [],
      );
      if (conditionFilter === "aktif")
        cumulative = cumulative.filter((student) => student.is_active);
      if (conditionFilter === "perlu_perhatian")
        cumulative = cumulative.filter((student) => student.alpha_count > 0);
      if (conditionFilter === "stabil")
        cumulative = cumulative.filter((student) => student.alpha_count === 0);
                  ...
                  <div className="mt-3">
                    ...
                    >
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <PopoverHeader>
                          <PopoverTitle>Pilih tanggal absensi</PopoverTitle>
                        </PopoverHeader>
                        <Calendar
                          mode="single"
                          ...
                          onSelect={(date) => {
                            if (date) {
                              setSpecificDate(toDateInputValue(date));
                              setConditionFilter(null);
                              setSortBy(null);
                              setSpecificDateOpen(false);
                            }
                ...
                {dateMode === "range" && (
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    ...
                    >
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <PopoverHeader>
                          <PopoverTitle>Tanggal mulai</PopoverTitle>
                        </PopoverHeader>
                        <Calendar
                          mode="single"
                          ...
                          onSelect={(date) => {
                            if (date) {
                              setRangeFrom(toDateInputValue(date));
                              setConditionFilter(null);
                              setSortBy(null);
                              setRangeFromOpen(false);
                            }
                    ...
                    <Popover open={rangeToOpen} onOpenChange={setRangeToOpen}>
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <PopoverHeader>
                          <PopoverTitle>Tanggal sampai</PopoverTitle>
                        </PopoverHeader>
                        <Calendar
                          mode="single"
                          ...
                          onSelect={(date) => {
                            if (date) {
                              setRangeTo(toDateInputValue(date));
                              setConditionFilter(null);
                              setSortBy(null);
                              setRangeToOpen(false);
                            }
              ...
              >
                {reportType === "daily" ? (
                  <div className="grid gap-2 sm:grid-cols-3">
                    ...
                    />
                    <ReportCheckbox
                      checked={dailyColumns.nis}
                      onChange={(value) =>
                        setDailyColumns((current) => ({
                          ...current,
                          nis: value,
                        }))
                      }
                  ...
                  <div className="grid gap-2 sm:grid-cols-3">
                    ...
                    <ReportCheckbox
                      ...
                      onChange={(value) =>
                        setColumns((current) => ({ ...current, gender: value }))
                      }
                      label="Gender"
                    />
                    <ReportCheckbox
                    ...
                    <ReportCheckbox
                      ...
                      onChange={(value) =>
                        setColumns((current) => ({ ...current, hadir: value }))
                      }
                      label="Hadir"
                    />
                    <ReportCheckbox
                      ...
                      onChange={(value) =>
                        setColumns((current) => ({ ...current, izin: value }))
                      }
                      label="Izin"
                    />
                    <ReportCheckbox
                      ...
                      onChange={(value) =>
                        setColumns((current) => ({ ...current, sakit: value }))
                      }
                      label="Sakit"
                    />
                    <ReportCheckbox
                      ...
                      onChange={(value) =>
                        setColumns((current) => ({ ...current, alfa: value }))
                      }
                      label="Alfa"
                    />
                    <ReportCheckbox
                      ...
                      onChange={(value) =>
                        setColumns((current) => ({ ...current, status: value }))
                      }
                      label="Status"
                    />
                  </div>
