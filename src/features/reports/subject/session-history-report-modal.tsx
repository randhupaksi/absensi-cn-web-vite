  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <ReportModalFooter
          canDownload={Boolean(
            format && sortBy && hasData && !recapsQuery.isLoading,
          )}
...
async function generateSubjectRecapPdf(
  ...
  reports.forEach((report, index) => {
    ...
    body.push(
      head[0].map((header, headerIndex) => ({
        content:
          headerIndex === 1
            ? "Total"
            : { H: totals.h, I: totals.i, S: totals.s, A: totals.a }[header] !==
                undefined
              ? String(
                  { H: totals.h, I: totals.i, S: totals.s, A: totals.a }[
                    header
                  ],
                )
              : "",
  ...
  doc.save(
    `Laporan-Rekap-Sesi-Mapel-${new Date().toISOString().slice(0, 10)}.pdf`,
  );
}
