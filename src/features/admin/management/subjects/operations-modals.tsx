...
export function ScheduleOverrideModal({
  ...
  return (
    <PremiumModal
      ...
      icon={CalendarSync}
      title={item ? "Ubah Perubahan Jadwal" : "Tambah Perubahan Jadwal"}
      description="Batalkan, jadwalkan ulang, atau tetapkan guru pengganti untuk satu tanggal."
      ...
      footer={
        <Footer
          ...
          submitLabel={item ? "Update Jadwal" : "Simpan Jadwal"}
        />
      }
    >
      <form
      ...
      >
        <SelectField
          label="Jadwal"
          value={form.watch("schedule_id")}
          onChange={(value) =>
            form.setValue("schedule_id", value, { shouldValidate: true })
          }
          options={schedules.map((x) => ({
            value: x.id,
            label: `${x.subject_code} · ${x.class_name}`,
            description: `${x.teacher_name} · ${x.hari} ${x.jam_mulai}-${x.jam_selesai}`,
          }))}
          error={form.formState.errors.schedule_id?.message}
        />
        <SelectField
          ...
          onChange={(value) =>
            form.setValue(
              "override_type",
              value as ScheduleOverrideFormValues["override_type"],
            )
          }
          options={[
          ...
          error={form.formState.errors.override_type?.message}
        />
        <DateField
          control={form.control}
          name="original_date"
          label="Tanggal Asal"
          error={form.formState.errors.original_date?.message}
        />
        <SelectField
          label="Status"
          ...
          onChange={(value) =>
            form.setValue(
              "status",
              value as ScheduleOverrideFormValues["status"],
            )
          }
          options={[
            { value: "ACTIVE", label: "Aktif" },
            { value: "APPLIED", label: "Sudah Diterapkan" },
            { value: "CANCELLED", label: "Dibatalkan" },
          ]}
        ...
        {type === "RESCHEDULED" ? (
          <>
            <DateField
              ...
              error={form.formState.errors.replacement_date?.message}
            />
            <div className="grid grid-cols-2 gap-3">
              <div className={premiumModalFieldClassName}>
                <label className={premiumModalLabelClassName}>Mulai</label>
                <Input
                  type="time"
                  className={inputClass}
                  {...form.register("replacement_start_time")}
                />
              </div>
              <div className={premiumModalFieldClassName}>
                <label className={premiumModalLabelClassName}>Selesai</label>
                <Input
                  type="time"
                  className={inputClass}
                  {...form.register("replacement_end_time")}
                />
              </div>
            </div>
          </>
        ...
        {type === "SUBSTITUTE" ? (
          <SelectField
            ...
            value={form.watch("substitute_teacher_id")}
            onChange={(value) =>
              form.setValue("substitute_teacher_id", value, {
                shouldValidate: true,
              })
            }
            options={teachers
            ...
            error={form.formState.errors.substitute_teacher_id?.message}
          />
        ) : null}
...
function SelectField({
  ...
  const searchable =
    label.toLowerCase().includes("guru") ||
...
function DateField({
  ...
  control: Control<ScheduleOverrideFormValues>;
  name: "original_date" | "replacement_date";
  label: string;
  error?: string;
}) {
  return (
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span
            className={`truncate ${value ? "text-slate-700" : "text-slate-400"}`}
          >
            {value
...
function overrideValues(
  ...
  return {
    ...
    override_type:
      item?.override_type === "RESCHEDULED" ||
      item?.override_type === "SUBSTITUTE"
        ? item.override_type
        : "CANCELLED",
    replacement_date: item?.replacement_date ?? "",
    replacement_start_time: item?.replacement_start_time ?? "",
    replacement_end_time: item?.replacement_end_time ?? "",
