...
import {
  countEffectiveHolidayDays,
  formatHolidayPeriod,
  holidayDurationDays,
  holidayPeriodState,
  jakartaDateKey,
} from "@/features/admin/management/holidays/date-utils";
import {
  ActionButtons,
  AddButton,
  DataTable,
...
import {
  createAdminSchoolHoliday,
  deleteAdminSchoolHoliday,
  updateAdminSchoolHoliday,
} from "@/services/admin.service";
import type {
  AdminSchoolHoliday,
  AdminSchoolHolidayPayload,
  AdminSchoolHolidayType,
} from "@/types/admin";
...
export function HolidayManagementSection({
  holidays,
  isLoading = false,
  errorMessage,
}: Props) {
  ...
  const [editingItem, setEditingItem] = useState<AdminSchoolHoliday | null>(
    null,
  );
  const [deleteTarget, setDeleteTarget] = useState<AdminSchoolHoliday | null>(
    null,
  );
  ...
  const filteredHolidays = useMemo(() => {
    ...
    return orderedHolidays.filter((item) => {
      const matchesType =
        typeFilter === "all" || item.holiday_type === typeFilter;
      const matchesStatus =
        statusFilter === "all" ||
        (statusFilter === "active" ? item.is_active : !item.is_active);
      const matchesQuery =
        !normalized ||
        `${item.name} ${item.description ?? ""} ${TYPE_LABEL[item.holiday_type]}`
          .toLowerCase()
          .includes(normalized);
  ...
  const upcomingHolidays = activeHolidays.filter(
    (item) => item.start_date > today,
  ).length;
  const activeDaysThisYear = countEffectiveHolidayDays(
    activeHolidays,
    currentYear,
  );
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      payload: AdminSchoolHolidayPayload;
    }) => updateAdminSchoolHoliday(id, payload),
    onSuccess: () => {
      toast.success("Periode libur berhasil diperbarui.");
      setEditingItem(null);
      invalidate();
    },
    onError: (error: Error) => toast.error(error.message),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Periode libur berhasil dihapus.");
  ...
  const toPayload = (
    values: SchoolHolidayFormValues,
  ): AdminSchoolHolidayPayload => ({ ...values });
  ...
  return (
    <>
      <section className="relative overflow-hidden rounded-[30px] border border-white/75 bg-[linear-gradient(180deg,rgba(255,255,255,0.96)_0%,rgba(250,253,252,0.94)_52%,rgba(245,252,249,0.96)_100%)] p-4 shadow-[0_28px_80px_rgba(28,77,61,0.1)] backdrop-blur-xl dark:border-slate-700 dark:bg-slate-950 dark:shadow-none sm:p-5 lg:p-6">
        <div className="relative border-b border-slate-200/80 pb-5 dark:border-slate-700">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="max-w-2xl text-[15px] leading-7 text-slate-600 sm:text-base">
                  Kelola libur nasional, cuti bersama, dan libur sekolah sebagai
                  satu kebijakan absensi terpusat.
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3 rounded-[22px] border border-slate-200/75 bg-white/76 px-4 py-4 shadow-[0_10px_24px_rgba(15,23,42,0.04)]">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-800">
                    Berlaku ke seluruh sistem
                  </p>
                  <p className="text-xs leading-5 text-slate-500">
                    Tanggal aktif tidak menghasilkan alfa dan tidak dihitung
                    dalam rekap kehadiran.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 xl:grid-cols-4">
            ...
            />
            <StatCard
              label={`Hari Efektif ${currentYear}`}
              value={activeDaysThisYear}
              icon={CalendarCheck2}
        ...
        {errorMessage ? (
          <div className="relative mt-5">
            <EmptyState
              icon={CalendarDays}
              title="Kalender libur belum bisa dimuat"
        ...
        <div className="relative mt-5">
          <DataTableCard
            ...
            emptyTitle="Periode libur tidak ditemukan"
            emptyDescription="Ubah filter atau tambahkan periode libur baru."
            icon={CalendarDays}
          ...
          >
            <DataTable>
              <DataTableHeadRow
                ...
                centerLabels={["Kategori", "Durasi", "Status"]}
              />
              <DataTableBody>
                {pageItems.map((item) => (
                  <DataTableRow key={item.id}>
                    <DataTableCell>
                      <p className="font-semibold text-slate-800">
                        {formatHolidayPeriod(item)}
                      </p>
                      <p className="mt-1 text-xs text-slate-400">
                        {holidayPeriodState(item, today)}
                      </p>
                    </DataTableCell>
                    <DataTableCell className="max-w-[340px] whitespace-normal">
                      <p className="font-semibold text-slate-800">
                        {item.name}
                      </p>
                      <p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-500">
                        {item.description || "Tanpa catatan tambahan"}
                      </p>
                    </DataTableCell>
                    <DataTableCell className="text-center">
                      <HolidayTypeBadge type={item.holiday_type} />
                    </DataTableCell>
                    <DataTableCell className="text-center font-semibold text-slate-700">
                      {holidayDurationDays(item)} hari
                    </DataTableCell>
                    <DataTableCell className="text-center">
      ...
      <DeleteConfirmationModal
        ...
        title="Hapus periode libur?"
        description={
          deleteTarget
            ? `${deleteTarget.name} (${formatHolidayPeriod(deleteTarget)}) akan dihapus dari kalender.`
            : "Periode libur akan dihapus."
        }
        warning="Tanggal tersebut akan kembali dianggap sebagai hari sekolah dan dapat masuk ke perhitungan absensi."
...
function Filter({
  ...
  return (
    <div className={`w-full ${wide ? "sm:w-[220px]" : "sm:w-[170px]"}`}>
      <RadixSelectField
        value={value}
        onValueChange={onChange}
        options={options}
        placeholder={placeholder}
        triggerClassName="h-14 rounded-[22px]"
      />
    </div>
  );
}

function HolidayMobileList({
  items,
  onEdit,
  onDelete,
  isDeletePending,
  ...
  onEdit: (item: AdminSchoolHoliday) => void;
  onDelete: (item: AdminSchoolHoliday) => void;
      ...
      {items.map((item) => (
        <MobileDataCard key={item.id}>
          <MobileDataHeader
            ...
            title={item.name}
            subtitle={formatHolidayPeriod(item)}
            badge={<ActiveBadge active={item.is_active} />}
          />
          <div className="mt-4 space-y-3">
            <MobileDataField
              label="Kategori"
              value={<HolidayTypeBadge type={item.holiday_type} />}
            />
            <MobileDataField
              label="Durasi"
              value={`${holidayDurationDays(item)} hari`}
            />
            <MobileDataField label="Catatan" value={item.description || "-"} />
          </div>
          <MobileDataFooter>
            <ActionButtons
              onEdit={() => onEdit(item)}
