...
} from "@/lib/validations/student-profile-schema";
...
const EMPTY_FORM: StudentProfileFormValues = {
  name: "",
  nis: "",
  nisn: "",
  password: "",
...
export function StudentProfileCreateModal({
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    ...
    if (!nextOpen) form.reset(EMPTY_FORM);
  };
  return (
    <StudentFormModal
      open={open}
      ...
      description="Buat akun, profil, dan penempatan kelas siswa dalam satu proses."
      icon={UsersRound}
      form={form}
      classes={classes}
      isPending={isPending}
...
export function StudentProfileEditModal({
  ...
  isPending,
  onSubmit,
}: CommonProps & { student: AdminStudent | null; currentClassId?: string }) {
  const defaults: StudentProfileFormValues = student
    ? {
        name: student.name,
        nis: student.nis,
        nisn: student.nisn ?? "",
  ...
  return (
    <StudentFormModal
      ...
      description="Perbarui akun, profil, dan kelas siswa tanpa menghapus histori."
      icon={FilePenLine}
      form={form}
      classes={classes}
      isPending={isPending}
      onSubmit={onSubmit}
      submitLabel="Update Siswa"
    />
  );
}
...
function StudentFormModal({
  ...
  form: ReturnType<typeof useForm<StudentProfileFormValues>>;
  submitLabel: string;
}) {
  ...
  useEffect(() => {
    if (!open) return;
    const initialClass = activeClasses.find(
      (item) => item.id === form.getValues("class_id"),
    );
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <FieldGroup label="Password Login">
            <Input
              ...
              placeholder="Masukkan password; kosongkan saat edit"
              className="h-14"
            />
          ...
          <FieldGroup label="NIS">
            <Input
              ...
              className="h-14"
            />
            <FieldError message={errors.nis?.message} />
          </FieldGroup>
            ...
            render={({ field }) => (
              <RadixSelectField
                ...
                onValueChange={(classId) => {
                  ...
                  const selected = activeClasses.find(
